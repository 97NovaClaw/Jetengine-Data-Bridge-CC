...
**[Site config, 2026-07-18 — Snippet 9 rev 7 + Snippet 18, no plugin change]** Split price range by variant type (staging decision): mosaic price displays now render "Completed Work" (physical_variations rows) and "Instructions" (pdf_variations rows) as separate labeled ranges, each label on its own line above its range. One code path — the parent branch of the `woocommerce_get_price_html` filter — serves BOTH the archive card (snippet 6 calls `get_price_html()`) and the product-page range line. Type identification via new Snippet 18 helper `bbhq_variation_row_type()`: the variation's `_jedb_variation_slug` UUID is located in either the physical_variations or pdf_variations repeater. Rules preserved: parent `display_price_publicly=no` still short-circuits to "Quote on request" before the split; `hide_price=yes` rows are excluded from ranges (a type whose rows ALL hide price shows "Quote on request" under its label); a type with no rows is omitted; single price renders without a dash range. Verified live on Koala, Lion, Orca (both groups render) and VT 3 (toggle=no gate intact).

**[Site config, 2026-07-12 — Snippet 9 rev 6, no plugin change]** Out-of-stock mosaics stay orderable (staging decision): the cart is a "Request Cart" quote flow, so stock ≤ 0 must not block add-to-cart. Implemented as RUNTIME filters only — stored stock data untouched, so two-way stock sync and the "Request a Commission" price display (which read raw stock) are unaffected. Filters: `woocommerce_is_purchasable` (published mosaic variations purchasable regardless of stock), `woocommerce_product_is_in_stock` + `woocommerce_variation_is_in_stock` (forced true for mosaics), `woocommerce_product_backorders_allowed` (passes cart quantity validation), `woocommerce_get_availability_text` (suppresses "Out of stock" notices — the price line communicates state). Verified live: variation at stock 0 → real `WC()->cart->add_to_cart()` SUCCESS, price still "Request a Commission", raw status still `outofstock`, reverse stock sync still mirrors to CCT.

**[Site config, 2026-07-12 — Snippet 9 rev 5, no plugin change]** Additional Info tab refinement per staging review: (1) "Approximate Size" row REMOVED — WC's native Dimensions row already carries per-variation LWH, so it was redundant. (2) "Stud Count" now behaves like Dimensions: the default row shows the range across enabled physical rows (e.g. "5,100 – 5,200 studs"), and a `wp_footer` JS hook swaps in the exact count when a variation is selected (payload travels on `available_variation.bbhq_stud_count_html`; PDF/no-stud variations restore the range). Row omitted entirely when no repeater row has a stud count. Verified live on Koala (2 physical rows + 1 PDF).

**[Site config, 2026-07-12 — Snippet 9 rev 4, no plugin change]** Decision-D refinement per staging review: the parent product's price-RANGE line (under the product title, Elementor Pro price widget → `get_price_html()`) now follows `display_price_publicly`, same rule as the archive card — `no` → "Quote on request". Scope: the parent toggle gates the RANGE line only. A selected variation still reveals its own price above add-to-cart per its `hide_price`/stock rules (hide_price=yes → "Quote on request"; stock ≤ 0 → "Request a Commission"; else price). New cached helper `bbhq_mosaic_price_public()`. Verified live: toggle=no parents render "Quote on request" while their variations still resolve individually (e.g. Koala: range hidden, 24x35 → "Quote on request" via hide_price, Instructions PDF → $24). Note: several test mosaics show an EMPTY range line because none of their repeater rows carry a Regular Price (pre-retirement parent price was 0, so backfill skipped them) — filling row prices fixes those, it's data not code.
