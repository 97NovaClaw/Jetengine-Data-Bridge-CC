...
- **Phase 4c-B — stock pull (WC → CCT): ✅ SHIPPED v0.6.0-alpha.24 (2026-07-12).** `woocommerce_variation_set_stock` handler (covers admin edits + purchase decrements), row located by UUID probe, surgical serialized-array write via direct SQL, push-lock echo suppression, sync_log origin `wc_variation_stock`. Stock fully two-way.
